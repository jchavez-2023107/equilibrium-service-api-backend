...
// Notificación general a todos los voluntarios (por botón de emergencia)
...
export const takeEmergencyNotification = async (req, res, next) => {
  try {
    ...
    if (!notification) {
      return res.status(404).json({ success: false, message: "Emergencia ya fue tomada o no existe" });
    }
...
export const createEmergencyAlertNotification = async (chat, requesterId) => {
  const volunteers = await User.find({ role: "VOLUNTEER", status: "ACTIVE" }, "_id");
